...
from prost.db import DrinkType, ProstDB, User
...
class TestPurchaseOperations:
    ...
    def test_purchase_reduces_stock(self, populated_db: tuple[ProstDB, dict]):
        ...
        initial_cola = next(
            item for item in initial_stock if item["drink_name"] == "Cola"
        )
        ...
        updated_cola = next(
            item for item in updated_stock if item["drink_name"] == "Cola"
        )
